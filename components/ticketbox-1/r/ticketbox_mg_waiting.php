...
function ticketbox_mg_waiting(){
	
	# 
	# actions
	switch($_REQUEST['do']){
		
		case 'ticketbox_mg_form':
			ticketbox_mg_form();
			return true;
		
		case 'ticketbox_mg_remove':
			ticketbox_mg_remove();
			break;
	...
	# 
	# list
	$list['query'] = " SELECT * FROM `ticketbox` WHERE `archived`='0' ORDER BY `date` DESC ";
	...
	$list['list_array'] = array(
		...
		#
		# date
		array(	"content" => 'substr(time_inword($rw[\'date\']), 0, 16)' , "attr" => array("width" => "120px" , "align" => 'center',"dir" => _ltr) ),
