...
function user_register_form(){
	...
	if( is_component('userregisterverify') and !userregisterverify_check() ){
		...
		if( is_component('userregisterverify') ){
			...
			if( $qpop = que::pop('result_of_userregisterverify_check') ){
				$qpop = explode( ':', $qpop );
				$qp_column = $qpop[0];
				$qp_value = $qpop[1];
		...
		switch ($_REQUEST['do']) {
			case 'saveNew':
				...
				# age doros anjam nashode, que::push kon moshkel ro, k tuye form 'prompt' beshe ruye element
				if( user_register_do() ){
					return true;
				}
				break;
		...
		echo listmaker_form('
			...
			[!"table" => "user@" , "action" => "'.layout_link(58).'" , "mobile_login" => "'.(userlogin_username_mobile ? 1 : 0).'" !]
				...
				[!"name:name*"=>$_REQUEST["name"],"prompt"=>que::pop("user_register_form_name")!]
				[!"number:cell.placeholder_fix"=>$_REQUEST["cell"],"prompt"=>que::pop("user_register_form_cell")!]

				[!"email:email.placeholder_fix'.( userlogin_username_mobile ? '/required=1' : '' ).'"=>$_REQUEST["email"],"prompt"=>que::pop("user_register_form_email")!]
				[!"password:password.placeholder_fix*","prompt"=>que::pop("user_register_form_password")!]
