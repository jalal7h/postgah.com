...
function abusereport_mg_view(){
	...
	# no report related to this id
		...
		if(! $rw_item ){
			echo "<div class=\"case removed\">گزارش تخلف $item_title به شناسه ".$rw_ar['table_id']." - قبلا حذف شده است.</div>";
		
		} else {

			$func_link = $rw_ar['table_name']."_link";

			echo "<div class=\"case\">گزارش تخلف در مورد $item_title: ".( function_exists($func_link) ? "<a href=\"".$func_link($rw_item)."\" target=\"_blank\" >".$rw_item['name']."</a>" : $rw_item['name'] )."</div>";
		...
		if( $rw_theGoodUser ){
			...
			echo "گزارش از طرف <a href=\"".$GoodUserLink."\" target=\"_blank\">".$rw_theGoodUser['name']."</a> (<a href=\"".users_loginLink($rw_theGoodUser['id'])."\" target=\"_blank\">ورود</a>)";
		}
		...
		if( $rw_theBadUser ){
			...
			echo "$item_title ثبت شده توسط <a href=\"".$BadUserLink."\" target=\"_blank\">".$rw_theBadUser['name']."</a> (<a href=\"".users_loginLink($rw_theBadUser['id'])."\" target=\"_blank\">ورود</a>)";
		}
		...
		if( $rw_item ){
			...
			echo "<a class=\"submit_button red\" href=\"".$remove_item_link."\" >حذف $item_title</a>";
		...
		if( $rw_theBadUser ){
			...
			echo "<a class=\"submit_button red\" href=\"".$removeUserItems_link."\" >حذف همه ".$item_title_s."ی این کاربر</a>";
			
			#
			# remove bad user and all of his things
			$removeBadUser_link = $base_link."&do=remove_user&id=".$rw_ar['id']."&user_id=".$rw_theBadUser['id'];
			echo "<a class=\"submit_button red\" href=\"".$removeBadUser_link."\" >حذف کاربر ثبت‌کننده</a>";
		...
		echo "<a class=\"submit_button red\" href=\"".$remove_abusereport_link."\" >حذف این گزارش</a>";
